...
- **自动化验收依据与人机边界铁律（铁律）**：
  - **机器自动化边界（qam test）**：代码与契约质量验证统一使用 **`qam test .\app-slug`**（秒级退出并输出确定性退出码），仅检验源码语法错误、模板动态编译、`v-cloak` 脱落与 IPC 契约接口，绝不外推声称用户的真实交互体验已验证；
  - **视觉自查第一铁律（视觉防呆）**：`qam test` 通过后，在交付用户试用或生成商店素材前，**必须强制执行 `qam screenshot .\app-slug` 捕获真机截图进行视觉自检**：
  ...
  - **交互与持久化唯一真理源（用户亲自体验铁律）**：
    - **严禁 Agent 假装真人代打**：严禁申请系统桌面控制权限、模拟物理键鼠或在窗口中伪造测试数据；
    - 真实交互体验、手感、功能可用性与断电/重启持久化，**必须且只能由用户在屏幕窗口中亲自试用与验证**。
