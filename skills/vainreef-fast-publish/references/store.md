...
- 模型缺少图片读取能力时，直接打开素材文件夹，不做视觉评价；OCR 文字与图片文件存在均不代表看过布局。开发和发布统一遵循 [验收边界](acceptance.md)。
...
第 2 步：上架材料全面盘点与来源协同 (向用户列出文案、图片规格，确认由 Agent 生成还是用户提供)
  ↓
第 3 步：无头界面素材生成、交互式弹窗与用户检视确认 (纯中文命名、纯文本 txt、置顶展示并等待用户回复确认)
  ↓
...
26. **全套商店提审资产规格清单（非占位图）**：必须使用无头界面高清截图与图标，全套包含：
...
29. **代表性截图与能力分支**：通过 `qam screenshot` 生成代表性状态供用户检查，空状态不替代带内容、带图状态；纯文字模型依据程序和文字证据报告，不作视觉通过结论。有图像能力且实际读取时可补充检查，用户最终确认。非首屏使用现有 `--eval` / `--click`，不为截图临时篡改业务源码，不污染正式用户数据。无头截图使用模拟桥接，不代表真实窗口与持久化验收；
