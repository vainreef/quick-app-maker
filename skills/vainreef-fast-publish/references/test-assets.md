...
> [!CAUTION]
> ### 【绝对红线】严禁一切下载海外资源的尝试！
> 1. **目标机器 100% 运行在中国大陆境内，无境外代理，直连海外公网必被墙、必超时卡死！** 严禁任何心存侥幸的海外直连尝试（包括但不限于 GitHub releases、raw.githubusercontent.com、海外未镜像 API 与 CDN）。
> 2. **如果需要下载某项资源，必须使用已验证的国内镜像/直链（如下方清单），或者明确搜索到可靠的中国大陆国内资源。**
> 3. **如果找不到国内可用资源，必须立刻彻底放弃下载该资源！严禁境外直连，必须换用本地生成、系统自带或 XAML 控件拟态等替代方案！**

> **环境约束**：Windows 目标实机默认只有 **git + curl.exe + PowerShell 5.1**（Windows 自带，**无 Python / node.js / ffmpeg**）。
> **网络环境**：所有外部资源必须在中国大陆网络直连畅通（全部使用国内 CDN、国内镜像、阿里云 OSS、清华 TUNA、Gitee 及 npmmirror）。
...
搜不到国内可用资源   → 立刻彻底放弃下载，换用本地方式（严禁境外直连）
```
