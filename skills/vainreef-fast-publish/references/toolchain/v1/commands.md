...
### 7. 【绝对红线】严禁任何海外资源下载尝试（中国境内网络，直连必被墙）

- **环境事实**：实机 100% 运行在中国大陆境内，无境外代理，直连 GitHub releases、raw.githubusercontent.com、海外 CDN / API **必定被墙或超时卡死**，导致会话严重挂起。
- **严禁心存侥幸**：绝对禁止直接访问任何境外未镜像站点尝试下载。
- **资源下载黄金准则**：
  1. 优先查阅 `test-assets.md` 已验证的国内源（Gitee 镜像、清华 TUNA、阿里云 OSS、npmmirror、img.scdn.io 等）。
  2. 需要新资源时，必须搜索到可靠的**中国大陆国内源**。
  3. **如果找不到国内可用资源，必须立刻彻底放弃下载！** 严禁尝试境外直连，必须换用本地生成（PowerShell + `System.Drawing` 绘图）、系统原生自带（`C:\Windows\Media\` 音效、`Segoe Fluent Icons` 字体图标、`PersonPicture` 控件）或 XAML 控件拟态等替代方案。
