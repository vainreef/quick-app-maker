# Official Sources

Use these pages when current commands or platform behavior require verification:

- [WinUI 3](https://learn.microsoft.com/en-us/windows/apps/winui/winui3/)
- [Windows App SDK](https://learn.microsoft.com/en-us/windows/apps/windows-app-sdk/)
- [Windows App Development CLI](https://learn.microsoft.com/en-us/windows/apps/dev-tools/winapp-cli/)
- [WinApp CLI command reference](https://learn.microsoft.com/en-us/windows/apps/dev-tools/winapp-cli/usage)
- [Debugging with package identity (`winapp run`)](https://learn.microsoft.com/en-us/windows/apps/dev-tools/winapp-cli/debugging)
- [Using winapp CLI with .NET](https://learn.microsoft.com/en-us/windows/apps/dev-tools/winapp-cli/guides/dotnet)
- [Windows App SDK self-contained deployment](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/self-contained-deploy/deploy-self-contained-apps)
- [MSIX signing end-to-end](https://learn.microsoft.com/en-us/windows/msix/package/sign-msix-package-guide)
- [MSIX troubleshooting, including 0x800B0109](https://learn.microsoft.com/en-us/windows/msix/msix-troubleshooting-guide)
- [Microsoft Store submission and signing model](https://learn.microsoft.com/en-us/windows/apps/publish/get-started)
- [Create an MSIX app submission](https://learn.microsoft.com/en-us/windows/apps/publish/publish-your-app/msix/create-app-submission)
- [Quickstart: Build and publish a Windows app with AI](https://learn.microsoft.com/en-us/windows/apps/develop/ai-assisted/quickstart)
- [.NET support policy](https://dotnet.microsoft.com/platform/support/policy)
- [System.Text.Json](https://learn.microsoft.com/en-us/dotnet/standard/serialization/system-text-json/overview)
- [SQLite data access in a Windows app](https://learn.microsoft.com/en-us/windows/apps/develop/data-access/sqlite-data-access)
- [Publish your first Windows app](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/publish-first-app)
- [Microsoft Store policies](https://learn.microsoft.com/en-us/windows/apps/publish/store-policies)
- [Code signing options for Windows app developers](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/code-signing-options)
- [App capability declarations](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/app-capability-declarations)
- [Set up Microsoft Marketplace payout and tax profiles](https://learn.microsoft.com/en-us/partner-center/account-settings/set-up-your-payout-account)
- [Open a Microsoft Store developer account](https://learn.microsoft.com/en-us/windows/apps/publish/partner-center/open-a-developer-account)
