...
> - **严格人机协同**：`store launch` 启动 Edge 后，Agent **必须向用户发送登录指引并等待用户回复「我登录好了」**，收到回复后再接力执行自动化填表。

```powershell
# 1. 启动独立 Edge 引导用户登录/注册 Partner Center（秒级返回，不阻塞）
& "$qamRoot\bootstrap\qam.cmd" store launch --app .\app-slug
# -> Agent 提示用户去窗口登录，并等待用户回复「我登录好了」

# 2. 用户回复「我登录好了」后，接力执行自动化流水线：
