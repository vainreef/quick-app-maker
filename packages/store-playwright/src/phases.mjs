...
export class StoreDriver {
  ...
  async captureEvidence(evidence, phase) {
    if (!evidence) return [];
    const ids = [];
    const base = phase.replace(/[^a-z0-9-]/gi, '-');
    const screenshot = path.join(evidence.dir, `${base}.png`); await this.page.screenshot({ path: screenshot, fullPage: false }); ids.push(evidence.write(`${base}.png`, fs.readFileSync(screenshot)));
...
async function save(page) { const button = page.getByRole('button', { name: /^(保存|Save|保存更改|Save changes)$/i }).last(); if (!(await button.count())) throw Object.assign(new Error('save button not found'), { code: 'SCHEMA_DRIFT' }); await button.click(); let becameDisabled = false; await waitUntil(async () => { const alerts = (await page.locator('[role="alert"],.alert-error,.alert-danger').allTextContents()).join(' '); if (/error|失败|错误/i.test(alerts)) throw new Error(`save returned an error: ${alerts}`); const body = await page.locator('body').innerText(); const disabled = await button.isDisabled().catch(() => false); becameDisabled ||= disabled; return /saved|已保存|保存成功/i.test(body) || disabled || (becameDisabled && !disabled); }, { timeoutMs: 30_000, label: 'save action settled' }); }
