...
Skill 收到新想法后先问“你想做什么”，让用户自由发挥创意。用户给出第一段实质想法后，Agent 立即在当前工作目录创建暂定项目文件夹和 living README；此后每轮先更新“用户需要什么、目前不需要什么、正在探索什么、已经确定什么”，再继续提问。用户表达“就是这样”之类的收口意图时，Agent 用通俗产品语言完整复述并询问“我理解的是这样，对吗？”，确认后再进入独立的可行性分析阶段。
